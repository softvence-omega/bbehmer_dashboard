...
const formSchema = z.object({
  maxLogsPerMonth: z.coerce
    .number()
    .min(0, 'Max Logs must be a non-negative number'),
  maxShareCount: z.coerce
    .number()
    .min(0, 'Max Share must be a non-negative number'),
  dataRetentionDays: z.coerce
    .number()
    .min(0, 'Data Retention must be a non-negative number'),
});
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Make changes to the plan details here. Click save when you're done.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            {errors.maxLogsPerMonth && (
              <p className="text-red-500 text-xs col-span-4 col-start-2">
                {errors.maxLogsPerMonth.message}
              </p>
            )}
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            {errors.dataRetentionDays && (
              <p className="text-red-500 text-xs col-span-4 col-start-2">
                {errors.dataRetentionDays.message}
              </p>
            )}
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
