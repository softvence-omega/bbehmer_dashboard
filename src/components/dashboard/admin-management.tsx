...
import { Loader2, Plus, Trash2, Edit, Eye, Search, AlertCircle, CheckCircle, ChevronsLeft, ChevronLeft, ChevronRight, ChevronsRight } from "lucide-react"
import { useCreateAdminMutation, useDeleteAdminMutation, useGetAdminManagementQuery, useUpdateAdminDetailsMutation } from "../../redux/features/admin/adminManagementApi"
    ...
    <div className="container mx-auto py-6 space-y-6 p-4">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {totalCount > 0 ? `Showing ${startItem} to ${endItem} of ${totalCount} admins` : "No admins found"}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoadingAdmins ? (
                ...
                {admins.length > 0 ? (
                  admins.map((admin: Admin) => (
                    <TableRow key={admin.id}>
                      ...
                      <TableCell>
                        <Badge className={admin.subscriptionPlan === "pro" ? "bg-purple-500" : "bg-gray-500"}>
                          {admin.subscriptionPlan.charAt(0).toUpperCase() + admin.subscriptionPlan.slice(1)}
                        </Badge>
                      </TableCell>
                      <TableCell>{formatDate(admin.createdAt)}</TableCell>
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          <Button size="icon" variant="ghost" onClick={() => openViewModal(admin)}>
                            <Eye className="h-4 w-4" />
                          </Button>
                          <Button size="icon" variant="ghost" onClick={() => openEditModal(admin)}>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={editForm.handleSubmit(handleUpdateAdmin)}>
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-password">New Password (leave unchanged to keep current)</Label>
                <Input
                  id="edit-password"
                  type="password"
                  placeholder="••••••••"
                ...
                <p className="text-xs text-gray-500">Leave as is to keep current password, or enter a new password</p>
              </div>
            </div>
            <DialogFooter>
              <Button
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDeleteAdmin} disabled={isLoading} className="bg-red-600 hover:bg-red-700">
              {isLoading ? (
                <>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  Deleting...
      ...
      <Dialog open={isViewModalOpen} onOpenChange={setIsViewModalOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Detailed information about this admin</DialogDescription>
          </DialogHeader>
          {selectedAdmin && (
              ...
              <TabsContent value="basic" className="space-y-4 pt-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{selectedAdmin.isAdmin ? "Admin" : "Not Admin"}</p>
                  </div>
                  <div>
                    <Label className="text-sm text-gray-500">Account Status</Label>
                    <p className="font-medium">{selectedAdmin.isSuspend ? "Suspended" : "Active"}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <Label className="text-sm text-gray-500">IP Ban Status</Label>
                    <p className="font-medium">{selectedAdmin.isIpBan ? "Banned" : "Not Banned"}</p>
                  </div>
                  <div>
              ...
              <TabsContent value="subscription" className="space-y-4 pt-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium capitalize">{selectedAdmin.subscriptionPlan}</p>
                  </div>
                  <div>
                    <Label className="text-sm text-gray-500">Plan Started</Label>
                    <p className="font-medium">{formatDate(selectedAdmin.planStartedAt)}</p>
                  </div>
                  <div>
                    <Label className="text-sm text-gray-500">Stripe Customer ID</Label>
                    <p className="font-medium">{selectedAdmin.stripeCustomerId || "Not available"}</p>
                  </div>
                  <div>
                    <Label className="text-sm text-gray-500">Stripe Subscription ID</Label>
                    <p className="font-medium">{selectedAdmin.stripeSubscriptionId || "Not available"}</p>
                  </div>
                </div>
              </TabsContent>
              <TabsContent value="activity" className="space-y-4 pt-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium">{selectedAdmin.isLogIn ? "Logged In" : "Logged Out"}</p>
                  </div>
                  <div>
                    <Label className="text-sm text-gray-500">Last Login</Label>
                    <p className="font-medium">{formatDate(selectedAdmin.lastLoginAt)}</p>
                  </div>
                  <div>
                    <Label className="text-sm text-gray-500">FCM Token</Label>
                    <p className="font-medium text-xs truncate">
                      {selectedAdmin.fcmToken ? selectedAdmin.fcmToken.substring(0, 20) + "..." : "Not available"}
                    </p>
                  </div>
                </div>
              </TabsContent>
