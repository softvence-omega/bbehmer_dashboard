...
const AnnouncementsList = () => {
  ...
  const {
    ...
    error,
    refetch,
  } = useAdminGetAllAnnouncementsQuery({ limit, offset });
  ...
  const totalPages = announcementsData?.data
    ? Math.ceil(announcementsData?.data?.length / limit)
    : 0;
    ...
    <div className="space-y-6 p-4">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm text-gray-400">
          Total: {announcementsData?.data?.length || 0} announcements
        </div>
      </div>
              ...
              {announcement.startsAt && announcement.endsAt && (
                <div className="mt-4 pt-4 border-t border-gray-800">
                  <div className="flex items-center gap-4 text-sm text-gray-400">
                    <span>
                      Active: {new Date(announcement.startsAt).toDateString()}
                    </span>
                    <span>
                      Until: {new Date(announcement.endsAt).toDateString()}
                    </span>
                  </div>
                </div>
              )}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          <div className="text-sm text-gray-400">
            Showing {offset + 1} to{' '}
            {Math.min(offset + limit, announcementsData?.total || 0)} of{' '}
